...
class Serializer
  ...
  constructor: (@attributes...) ->
    ...
    if @attributes[0]? and @attributes[0].fields? and @attributes[0].fields.length > 0
      @config.fields = _.clone @attributes[0].fields
    ...
    if @config.fields.length is 0 and @config.readOnlyFields.length is 0
      @serializerFields = _.keys @config.model::attributes
    else
      @serializerFields = _.union @config.fields, @config.readOnlyFields
    ...
    _.each @serializerFields, (val, key) =>
      ...
      if typeof val is 'string' and not @config.model::attributes[val]?
        throw new Error "Key '#{val}' does not match any attribute of model #{@config.model.metadata.model}"
    ...
    @serializerFieldsKeys = _.map @serializerFields, (val, key) =>
      ...
      if currentKey not in @config.excludeFields
        return currentKey

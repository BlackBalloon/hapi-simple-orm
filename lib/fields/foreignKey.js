...
(function() {
  'use strict';
  var BaseField, BaseModel, ForeignKey, Joi, _, fs, knex, knexConf,
  ...
  ForeignKey = (function(superClass) {
    ...
    function ForeignKey() {
      ...
      this.foreignKeyParameters = ['foreignKey', 'relatedName', 'referenceModel', 'referenceField', 'required'];
      this.foreignKeyValidationMethods = {
      ...
      if (!_.has(attributes[0], 'dbField')) {
        _.extend(attributes[0], this.getDbField(attributes[0].name));
      }
      if (!_.has(attributes[0], 'schema')) {
        if (_.has(attributes[0], 'required')) {
          ...
          _.extend(attributes[0], {
            'schema': Joi.number().integer().positive().allow(null)
          });
        }
      }
      this.acceptedParameters = _.union(this.constructor.acceptedParameters, this.foreignKeyParameters);
      this.validationMethods = _.extend(this.constructor.validationMethods, this.foreignKeyValidationMethods);
      ForeignKey.__super__.constructor.apply(this, arguments);
    }

    ForeignKey.prototype.validateForeignKey = function(value, arg) {
      var finalQuery, lookup, sqlQuery, trx;
      trx = (arg != null ? arg : {}).trx;
      if (value != null) {
        lookup = {};
        lookup[this.attributes.referenceField] = value;

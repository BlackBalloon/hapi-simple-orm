              ...
              return _this.config.model.objects().create({
                ...
                if (_this.config.createLogger != null) {
                  if ((request.auth.credentials != null) && (request.auth.credentials.user != null)) {
                    _this.config.createLogger.info(_this.config.model.metadata.model + " " + (JSON.stringify(result)) + " created by " + request.auth.credentials.user.username + ".");
                  } else {
                    _this.config.createLogger.info(_this.config.model.metadata.model + " " + (JSON.stringify(result)) + " created by anonymous.");
                  }
                }
                if (ifSerialize) {
                  serializerClass = serializer ? serializer : _this.config.serializer;
          ...
          handler: (function(_this) {
            return function(request, reply) {
              ...
              return _this.config.model.objects()["delete"](request.params.id, whoDeleted).then(function(result) {
                var publishObj;
                if (result === 1) {
                  if (_this.config.deleteLogger != null) {
                    if ((request.auth.credentials != null) && (request.auth.credentials.user != null)) {
                      _this.config.deleteLogger.info(_this.config.model.metadata.model + " with id " + request.params.id + " deleted by " + request.auth.credentials.user.username + ".");
                    } else {
                      _this.config.deleteLogger.info(_this.config.model.metadata.model + " with id " + request.params.id + " deleted by anonymous.");
                    }
                  }
                  publishObj = {
                    action: 'delete',

...
(function() {
  'use strict';
  var BaseField, BaseModel, ForeignKey, Joi, ManyToMany, ManyToOne, Promise, _, fs, knex, knexConf, moduleKeywords,
  ...
  BaseModel = (function() {
    ...
    BaseModel.objects = function() {
      var dao, daoFileName, modules, modulesDir, self;
      ...
      _.each(modules, function(moduleName) {
        var currentModule, insideModule, ref;
        if (!moduleName.startsWith('.')) {
          insideModule = fs.readdirSync(modulesDir + moduleName);
          if (indexOf.call(insideModule, 'dao') >= 0) {
            currentModule = fs.readdirSync(modulesDir + moduleName + '/dao');
            if (ref = daoFileName + '.coffee', indexOf.call(currentModule, ref) >= 0) {
              return dao = require(modulesDir + moduleName + '/dao/' + daoFileName);
            }
          }
        }
      });
      if (dao != null) {
        ...
        throw new Error("Model " + this.metadata.model + " does not have its DAO!");
      }
    };
    ...
    BaseModel.getSchema = function(fields, partial) {
      ...
      if (fields == null) {
        fields = _.keys(this.prototype.attributes);
      }
      attributesWithSchema = _.pick(this.prototype.attributes, function(val, key) {
        return (val.attributes.schema != null) && !(key in BaseModel.prototype.timestampAttributes) && indexOf.call(fields, key) >= 0;
      });
    ...
    function BaseModel(properties) {
      ...
      _.each(this.attributes, (function(_this) {
        return function(val, key) {
          ...
          if (!_.has(val.attributes, 'dbField')) {
            val.attributes.dbField = _this.constructor._camelToSnakeCase(key, _this.attributes);
          }
          if (!_.has(val.attributes, 'name')) {
          ...
          if (val instanceof ManyToOne) {
            ...
            _this[key] = new manyToOneManager(_this, key, val);
          }
          return val.attributes.modelMeta = _this.constructor.metadata;
        };
      })(this));
      try {
        this.set(properties);
        ...
        return function(validationResult) {
          ...
          if (_this[_this.constructor.metadata.primaryKey] != null) {
            ...
            return _this.constructor.objects().create({
              payload: _this._toDatabaseFields(),
              returning: returning,
              toObject: toObject
            }).then(function(res) {
              return res;
